  ...
  subroutine ch4conrd ()
    ...
    if (masterproc) then
       ...
       if (ierr == 0) then
          ...
          if (ierr /= 0) then
             call endrun('error in reading in ch4par_in namelist'//&
                  errMsg(sourcefile, __LINE__))
          end if
       else
          call endrun('error in finding ch4par_in namelist'//&
                  errMsg(sourcefile, __LINE__))
       end if
       call relavu( unitn )
       ...
       if (      trim(finundation_method) == "h2osfc"        )then
         ...
         finundation_mtd = finundation_mtd_zwt_inversion
          ...
          call endrun('error bad value for finundation_method in ch4par_in namelist'//&
                  errMsg(sourcefile, __LINE__))
       end if
