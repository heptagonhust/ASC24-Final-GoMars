...
contains
  ...
  subroutine div_damp_init()
    ...
    case (2)
      ...
      do k = global_mesh%full_kds, global_mesh%full_kde
        do j = global_mesh%full_jds_no_pole, global_mesh%full_jde_no_pole
          if (baroclinic) then
            c_lon(j,k) = div_damp_coef2 * global_mesh%full_cos_lat(j)**(r - 1) * &
      ...
      do k = global_mesh%full_kds, global_mesh%full_kde
        do j = global_mesh%half_jds, global_mesh%half_jde
          if (baroclinic) then
            c_lat(j,k) = div_damp_coef2 * global_mesh%half_cos_lat(j)**(r - 1) * &
              exp_two_values(div_damp_top, 1.0_r8, 1.0_r8, real(div_damp_k0, r8), real(k, r8)) * &
    ...
    case (4)
      ...
      do k = global_mesh%full_kds, global_mesh%full_kde
        do j = global_mesh%full_jds_no_pole, global_mesh%full_jde_no_pole
          c_lon(j,k) = div_damp_coef4 * global_mesh%full_cos_lat(j)**(r - 2) * &
      ...
      do k = global_mesh%full_kds, global_mesh%full_kde
        do j = global_mesh%half_jds, global_mesh%half_jde
          c_lat(j,k) = div_damp_coef4 * global_mesh%half_cos_lat(j)**(r - 2) * &
      ...
      do k = mesh%full_kds, mesh%full_kde
        do j = mesh%full_jds_no_pole, mesh%full_jde_no_pole
          do i = mesh%half_ids, mesh%half_ide
            dudt(i,j,k) = dudt(i,j,k) + c_lon(j,k) * (div(i+1,j,k) - div(i,j,k)) / mesh%de_lon(j) / dt
          end do
        end do
      end do
      do k = mesh%full_kds, mesh%full_kde
        do j = mesh%half_jds, mesh%half_jde
          do i = mesh%full_ids, mesh%full_ide
            dvdt(i,j,k) = dvdt(i,j,k) + c_lat(j,k) * (div(i,j+1,k) - div(i,j,k)) / mesh%de_lat(j) / dt
          end do
        end do
      end do
    case (4)
      do k = mesh%full_kds, mesh%full_kde
        do j = mesh%full_jds_no_pole, mesh%full_jde_no_pole
          do i = mesh%half_ids, mesh%half_ide
            dudt(i,j,k) = dudt(i,j,k) - c_lon(j,k) * (div2(i+1,j,k) - div2(i,j,k)) / mesh%de_lon(j) / dt
          end do
        end do
      end do
      do k = mesh%full_kds, mesh%full_kde
        do j = mesh%half_jds, mesh%half_jde
          do i = mesh%full_ids, mesh%full_ide
            dvdt(i,j,k) = dvdt(i,j,k) - c_lat(j,k) * (div2(i,j+1,k) - div2(i,j,k)) / mesh%de_lat(j) / dt
          end do
        end do
      end do
    end select
