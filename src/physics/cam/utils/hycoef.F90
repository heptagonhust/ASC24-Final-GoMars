...
use vert_coord_mod,   only: hyai, hybi, hyam, hybm
...
#endif
real(r8), target, allocatable :: alev(:)         ! level values (pascals) for 'lev' coord
real(r8), target, allocatable :: ailev(:)        ! interface level values for 'ilev' coord
...
subroutine hycoef_init(file, psdry)
   ...
   allocate(etamid(plev ))
   allocate(hybd  (plev ))
   allocate(hypi  (plevp))
   allocate(hypm  (plev ))
   allocate(hypd  (plev ))
   allocate(alev  (plev ))
   allocate(ailev (plevp))
