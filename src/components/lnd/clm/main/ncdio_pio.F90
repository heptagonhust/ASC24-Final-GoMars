...
module ncdio_pio
  ...
  use decompMod      , only : get_subgrid_level_from_name, get_subgrid_level_gsize, get_subgrid_level_gindex
...
# 178 "ncdio_pio.F90.in"
contains

  !-----------------------------------------------------------------------
# 181 "ncdio_pio.F90.in"
  subroutine ncd_pio_init()
    ...
    io_netcdf_format = shr_pio_getioformat(inst_name)
# 194 "ncdio_pio.F90.in"
...
# 219 "ncdio_pio.F90.in"
  end subroutine ncd_pio_openfile

  !-----------------------------------------------------------------------
# 222 "ncdio_pio.F90.in"
...
# 233 "ncdio_pio.F90.in"
  end subroutine ncd_pio_closefile

  !-----------------------------------------------------------------------
# 236 "ncdio_pio.F90.in"
...
# 261 "ncdio_pio.F90.in"
  end subroutine ncd_pio_createfile

  !-----------------------------------------------------------------------
# 264 "ncdio_pio.F90.in"
...
# 302 "ncdio_pio.F90.in"
  end subroutine check_var

  !-----------------------------------------------------------------------
# 305 "ncdio_pio.F90.in"
...
# 323 "ncdio_pio.F90.in"
  end subroutine check_dim

  !-----------------------------------------------------------------------
# 326 "ncdio_pio.F90.in"
...
# 412 "ncdio_pio.F90.in"
  end subroutine check_dim_size

  !-----------------------------------------------------------------------
# 415 "ncdio_pio.F90.in"
...
# 448 "ncdio_pio.F90.in"
  end subroutine ncd_enddef

  !-----------------------------------------------------------------------
# 451 "ncdio_pio.F90.in"
...
# 479 "ncdio_pio.F90.in"
  end subroutine ncd_inqdid

  !-----------------------------------------------------------------------
# 482 "ncdio_pio.F90.in"
...
# 507 "ncdio_pio.F90.in"
  end subroutine ncd_inqdlen

  !-----------------------------------------------------------------------
# 510 "ncdio_pio.F90.in"
...
# 650 "ncdio_pio.F90.in"
  end subroutine ncd_inqvtype

  !-----------------------------------------------------------------------
# 653 "ncdio_pio.F90.in"
...
# 691 "ncdio_pio.F90.in"
  end subroutine ncd_inqvname

  !-----------------------------------------------------------------------
# 694 "ncdio_pio.F90.in"
...
# 757 "ncdio_pio.F90.in"
  end subroutine ncd_inqvdlen_byDesc


  !-----------------------------------------------------------------------
# 761 "ncdio_pio.F90.in"
...
# 797 "ncdio_pio.F90.in"
  end subroutine ncd_inqvdlen_byName

  !-----------------------------------------------------------------------
# 800 "ncdio_pio.F90.in"
...
# 845 "ncdio_pio.F90.in"
  end subroutine ncd_inqvdname_byDesc

  !-----------------------------------------------------------------------
# 848 "ncdio_pio.F90.in"
...
# 906 "ncdio_pio.F90.in"
  end subroutine ncd_inqnatts

  !-----------------------------------------------------------------------
# 909 "ncdio_pio.F90.in"
...
# 928 "ncdio_pio.F90.in"
  end subroutine ncd_inqattname

  !-----------------------------------------------------------------------
# 931 "ncdio_pio.F90.in"
...
# 970 "ncdio_pio.F90.in"
  end subroutine ncd_putatt_char

  !-----------------------------------------------------------------------
# 973 "ncdio_pio.F90.in"
...
# 998 "ncdio_pio.F90.in"
  end subroutine ncd_putatt_real

  !-----------------------------------------------------------------------
# 1001 "ncdio_pio.F90.in"
...
# 1022 "ncdio_pio.F90.in"
  end subroutine ncd_getatt_char

    !-----------------------------------------------------------------------
# 1025 "ncdio_pio.F90.in"
...
# 1046 "ncdio_pio.F90.in"
  end subroutine ncd_getatt_int


  !-----------------------------------------------------------------------
# 1050 "ncdio_pio.F90.in"
...
# 1207 "ncdio_pio.F90.in"
  end subroutine ncd_defvar_bynf

  !-----------------------------------------------------------------------
# 1210 "ncdio_pio.F90.in"
...
# 1291 "ncdio_pio.F90.in"
  end subroutine ncd_defvar_bygrid

  !------------------------------------------------------------------------
# 1294 "ncdio_pio.F90.in"
...
# 1321 "ncdio_pio.F90.in"
  end subroutine ncd_io_char_var0_start_glob

  !------------------------------------------------------------------------
  !DIMS 0,1
# 1325 "ncdio_pio.F90.in"
...
# 1651 "ncdio_pio.F90.in"
  subroutine ncd_io_1d_int(varname, data, dim1name, flag, ncid, nt, readvar, cnvrtnan2fill)
    ...
    if (masterproc .and. debug > 1) then
       write(iulog,*) subname//' ',trim(flag),' ',trim(varname),' ',trim(subgrid_level_name)
    end if
    ...
    if (flag == 'read') then
       ...
       if (varpresent) then
          ...
          call ncd_getiodesc(ncid, subgrid_level_name, ndims_iod, dims(1:ndims_iod), dids(1:ndims_iod), &
               xtype, iodnum)
...
# 2559 "ncdio_pio.F90.in"
  subroutine ncd_getiodesc(ncid, subgrid_level_name, ndims, dims, dimids, &
    ...
    ! !ARGUMENTS:
    class(file_desc_t) , intent(inout) :: ncid       ! PIO file descriptor
    character(len=8)   , intent(in)    :: subgrid_level_name ! one of nameg, namel, etc.
    ...
    integer :: subgrid_level                 ! integer version of subgrid_level_name
    integer :: lsize                         ! local size of subgrid_level gindex
    integer :: gsize                         ! global size of subgrid_level
    ...
    subgrid_level = get_subgrid_level_from_name(subgrid_level_name)
    call get_subgrid_level_gindex(subgrid_level, gindex)
    if (.not. associated(gindex)) then
       call shr_sys_abort('gindex for subgrid level '//trim(subgrid_level_name)//' is not associated')
    end if
    gsize = get_subgrid_level_gsize(subgrid_level)
    lsize = size(gindex)
